...
conformance kit of golden vectors. It then separates integrity from
\emph{authenticity}, which no amount of hashing can supply: one detached
signature over a snapshot covers every module root and the entire prior history,
the keys authorized to produce one travel inside the brain as a trust root whose
revisions require a quorum of previously authorized keys, and a signature's
validity is decided by position in the hash-linked chain rather than by a
timestamp a signer could forge. Divergence becomes reconciliation rather than a
refusal: snapshots name several parents, compositions are merged as sets of
immutable blocks, and the result is re-validated as an ingestion, so a conflict
surfaces as a validation verdict and a contribution from outside requires no
object the protocol did not already have. We close with the adversarial model
this architecture implies, including what it does not close: first contact, a
registry that withholds newer snapshots, and untrusted evidence replayed into a
model by design.
\end{abstract}
...
record (Section~\ref{sec:authenticity}), which is why authenticity belongs in
this specification rather than in a deployment's configuration.
...
removes a single entry from an index, because indices are built and queried
rather than edited. Put together, an implementation that does the obvious thing
...
recover enough of their input for the concern to be practical rather than
theoretical~\cite{morris2023embeddings}. This is the \emph{ordinary} path, not
...
require a signature over any other object in order to establish authorship.
...
position that is already published, so its digest is known and no cycle arises;
and the finer granularity is necessary rather than incidental, since a revision
...
authority the brain will ever have, its rules are stated rather than left to be
inferred.
...
meet in an uncomfortable place, and this document states it rather than leaving
it to be discovered: for personal data inside an episode, redaction is the
...
Section~\ref{sec:forgetting}, and unifying them is a simplification rather than a
coincidence: both are legitimate only before publication, under version
...
party outside the artifact, which is why neither is specified here rather than
adopted casually: the offline property of
...
\begin{itemize}[leftmargin=1.4em]
  \item Recompute integrity rather than accept a publisher's assertion of it
  (Section~\ref{sec:verification}).
...
verification in order to be a conforming reader would make offline integrity
...
come from inside is trust at first contact, and that limitation is stated rather
...
signed makes the choice a question of attribution rather than of tidiness. The
