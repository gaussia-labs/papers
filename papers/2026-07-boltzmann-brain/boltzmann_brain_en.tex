...
Each canonical block carries at least its \texttt{block\_id}, media type, size,
license or retention policy, registering actor, and timestamp, and may carry a
\texttt{supersedes} link to an earlier canonical block when a new edition
replaces an old one (Section~\ref{sec:ingestion}).
...
the new original as a distinct block and records a \texttt{supersedes} link to
the previous one. The caller chooses whether the superseded original remains in
