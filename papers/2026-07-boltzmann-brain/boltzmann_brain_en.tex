...
snapshot can be read and extended by any conforming client. Because wrong
knowledge must be removable without punching holes in the current snapshot, we
also specify how non-episodic modules drop blocks, rebuild their Merkle DAGs,
and cascade the change through provenance, with redaction reserved for the
rarer case where retained bytes must be destroyed. This first version
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  cascade for non-episodic modules, supersession for episodic memory, pruning
  of unreachable blocks, and redaction when retained bytes must be destroyed.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  with Merkle rebuild and provenance cascade; content may be redacted under
  policy, but never silently.
\end{enumerate}
...
Dropping a block is not local. Provenance already answers which derived blocks
depend on a given source or transformation (Section~\ref{sec:modules}). When a
canonical block is dropped, the protocol walks that dependency closure and drops
or marks for re-derivation every semantic and procedural block that listed it
as evidence. When a semantic or procedural block is dropped, provenance removes
or rewrites the edges that pointed to it, and any downstream dependents are
treated the same way. The cascade is itself a set of drops: each affected
module receives a new Merkle root, and a single logical removal can therefore
publish several new module versions in one operation. Provenance also records
the actor, the policy, the reason, and the set of roots that resulted, so the
removal is auditable even though the dropped content is no longer part of the
current composition.
...
the block was never referenced. Redaction is the exception, not the cleanup
path for wrong semantic or procedural knowledge. Wrong knowledge is dropped; it
is not tombstoned.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  obsolete blocks, rebuild their Merkle roots, cascade through provenance, and
  prune what no retained root needs; episodic memory supersedes; redaction
  destroys bytes only when policy requires it (Section~\ref{sec:forgetting}).
\end{enumerate}
...
incremental updates, and an account of how non-episodic modules drop wrong
knowledge, rebuild Merkle roots, cascade through provenance, and reclaim
unreachable blocks, with redaction reserved for destroying retained bytes.
