...
\begin{verbatim}
{
  ...
  "keys": [
    { "key":          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5...",
      ...
      "since":        "sha256:SNAPSHOT_R1" },
    { "key":          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5...",
      "scopes":       ["ingest", "commit", "govern"],
      "since":        "sha256:SNAPSHOT_R1" },
    { "key":          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5...",
      "scopes":       ["propose"],
      "since":        "sha256:SNAPSHOT_R2" },
    { "key":          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5...",
      "scopes":       ["commit"],
      "since":        "sha256:SNAPSHOT_R1",
      "retired_from": "sha256:SNAPSHOT_R3" }
  ]
}
\end{verbatim}
...
snapshot and its first parent and MUST reject a signature whose key does not
hold every scope in that set, even if the signature claims fewer. The
...
revision} is therefore a snapshot whose \texttt{trust\_root} digest differs from
its first parent's, and it MUST satisfy two conditions: it MUST be covered by at
...
\begin{description}[leftmargin=2.2em, style=sameline, font=\normalfont\bfseries]
  \item[\texttt{retired\_from}] The position at which the key ceased to be
  ...
  \item[\texttt{compromised\_from}] A position from which the key's signatures
  are no longer trusted. It MAY precede the revision that records it, because
  compromise is usually discovered after the fact. This is the only construct in
  this document that invalidates a previously valid signature, and a verifier
  MUST report it as such rather than as an ordinary authorization failure.
\end{description}
